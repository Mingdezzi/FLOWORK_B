    ...
    toggleHorizontalMode(switchEl) {
        ...
        conditionalFields.forEach(wrapper => {
            ...
            if (isHorizontal) {
                ...
                // [수정] 숨길 때 required 속성 제거 (폼 제출 오류 방지)
                if (select) select.removeAttribute('required');
            } else {
                wrapper.style.display = 'block';
                // [수정] 보일 때 required 속성 복원 (필수 필드라면)
                // 여기서는 vertical 필드들이 모두 필수라고 가정하고 복원합니다.
                // 만약 선택적 필드라면 HTML에 data-required="true" 등을 둬서 체크해야 하지만
                // 현재 로직상 사이즈/재고는 필수이므로 추가합니다.
